const CARDS = [
  ...
  const drawFive = document.querySelector("#pick-five")
...
// Instructions //Create a new function (getCard) 
// Inside the function // Create random index 
// Grab a random card from the cards array 
// create new image element 
// set the 'src' attribute of the img use 
// setAttribute() 
// append the image the the deck 
// Add the function to the draw one buttons
const getCard = () => {
    let randomCard = CARDS[(Math.floor(Math.random() * CARDS.length))]
    const img = document.createElement('img')
    img.src = randomCard.image
    deckOfCards.appendChild(img)
}
...
drawFive.addEventListener('click', getFive)
